...
##############################################
## Simulation Setup for IR-SV-ACD(3,2)
##############################################

T <- 5020  # Number of time points
p <- 3  # Lag order for g
q <- 2  # Lag order for psi
k <- p+q+1
mxpq <- max(p,q)
...
sigma <- 0.8
mu <- -9
durpar <- c(0.05, 0.1, 0.15, 0.2, 0.4)
...
fit=model$sample(data=data_list,seed=1,chains = 4,iter_sampling = 1000,parallel_chains = 4,threads_per_chain = 12,iter_warmup=1000,thin=4,adapt_delta = 0.95)
...
filename1 <-sprintf("/Results/ppsampfull_g32.rds")
...
alpha_fit <- fit$draws(c("durpar[1]","durpar[2]", "durpar[3]"),format = "df") # change this and the next line as per the model order
betastar_fit <- fit$draws(c("durpar[4]","durpar[5]"),format = "df")
...
for(id in 1:nSamp){
  delta <- delta_fit[id]
  omega <- omega_fit[id]
  alpha <- as.numeric(alpha_fit[id,1:p])
  beta <- as.numeric(betastar_fit[id,1:q])
  mu <- mu_fit[id]
  sigma <- sigma_fit[id]
  phi <- phi_fit[id]
  T=5000
  ...
  for (t in (mxpq+1):(T)){
    psi_new[t] <- omega+as.numeric(alpha[1:p])%*%g[(t-1):(t-p)]+as.numeric(beta[1:q])%*%psi_new[(t-1):(t-q)]
    gppSamples_new[t] <- rgamma(n=1, shape=delta, scale=psi_new[t]/delta)
  }
...
for(id in 1:nSamp){
  ...
  for (t in 1:mxpq){
    psi_fore[t] <- omega+as.numeric(alpha[1:p])%*%g[(5000+t-1):(5000+t-p)]+as.numeric(beta[1:q])%*%psi_pp
    gppSamples_fore[t] <- rgamma(n=1, shape=delta, scale=psi_fore[t]/delta)
  }
  for (t in (mxpq+1):(T)){
    psi_fore[t] <- omega+as.numeric(alpha[1:p])%*%gppSamples_fore[(t-1):(t-p)]+as.numeric(beta[1:q])%*%psi_fore[(t-1):(t-q)]
    gppSamples_fore[t] <- rgamma(n=1, shape=delta, scale=psi_fore[t]/delta)
  }
